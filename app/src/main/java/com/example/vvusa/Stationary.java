...
public class Stationary extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pd.setMessage("Loading Stationaries, please wait...");
        pd.show();

        recyclerView = findViewById(R.id.Stationary);
    ...
    private void EventChangeListener() {
        db.collection("Stationary").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(@Nullable @org.jetbrains.annotations.Nullable QuerySnapshot value, @Nullable @org.jetbrains.annotations.Nullable FirebaseFirestoreException error) {
                if (error != null){
                    pd.dismiss();
                    Log.e("Error", error.getMessage());
                    Toast.makeText(Stationary.this, "Error loading gadgets, please try again", Toast.LENGTH_SHORT).show();
                    return;
                }
