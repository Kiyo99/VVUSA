...
//        pd.setCancelable(false);
        pd.setMessage("Loading Gadgets, please wait...");
        pd.show();

        recyclerView = findViewById(R.id.Gadgets);
    ...
    private void EventChangeListener() {
        db.collection("Gadgets").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(@Nullable @org.jetbrains.annotations.Nullable QuerySnapshot value, @Nullable @org.jetbrains.annotations.Nullable FirebaseFirestoreException error) {
                if (error != null){
                    pd.dismiss();
                    Log.e("Error", error.getMessage());
                    Toast.makeText(GadgetsActivity.this, "Error loading gadgets, please try again", Toast.LENGTH_SHORT).show();
                    return;
                }
