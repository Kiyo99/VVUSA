...
//        pd.setCancelable(false);
        pd.setMessage("Loading Bags, please wait...");
        pd.show();

        recyclerView = findViewById(R.id.Bags);
    ...
    private void EventChangeListener() {
        db.collection("Bags").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(@Nullable @org.jetbrains.annotations.Nullable QuerySnapshot value, @Nullable @org.jetbrains.annotations.Nullable FirebaseFirestoreException error) {
                if (error != null){
                    pd.dismiss();
                    Log.e("Error", error.getMessage());
                    Toast.makeText(BagsActivity.this, "Error loading Bags, please try again", Toast.LENGTH_SHORT).show();
                    return;
                }
